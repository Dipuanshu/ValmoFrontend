...
const BankDetails1 = () => {
  ...
  const fetchBankDetails = async () => {
    try {
      ...
      // Handle response whether it has success property or not
      if (res && res.data) {
        if (res.data.success) {
          ...
          // If success is false or not present, but we have data, still try to use it
          if (res.data.data) {
            const bankData = Array.isArray(res.data.data)
              ? res.data.data
              : [res.data.data];
    ...
    try {
      ...
      if (res && res.data) {
        if (res.data.success) {
          ...
          // Handle case where request succeeds but response indicates failure
          setStatus("Error saving bank details ❌");
          alert(res.data.message || "Failed to save bank details ❌");
        }
      } else {
        // Handle case where response is undefined or malformed
        setStatus("Error saving bank details ❌");
        alert("Failed to save bank details - unexpected response");
      }
    } catch (err) {
    ...
    try {
      ...
      if (res && res.data) {
        if (res.data.success) {
          ...
          // Handle case where request succeeds but response indicates failure
          alert(res.data.message || "Failed to delete bank details ❌");
        }
      } else {
        // Handle case where response is undefined or malformed
        alert("Failed to delete bank details - unexpected response");
      }
    } catch (err) {
  ...
  const handleQrUpload = async (bankId) => {
    ...
    try {
      ...
      // Check if upload response exists and handle errors
      if (!res || !res.data) {
        alert("Failed to upload QR - unexpected response");
        return;
      }
      ...
      if (!qrUrl) {
        alert("Failed to get QR URL from upload response");
        return;
      }
      ...
      // Check if update response exists and handle success/failure
      if (updateRes && updateRes.data) {
        ...
        alert(
          "Failed to update bank details with QR code - unexpected response"
        );
      }
    } catch (err) {
      ...
      if (err.response && err.response.data) {
        ...
        alert(`Error uploading QR code: ${err.message || "Unknown error"}`);
      }
    }
  };
